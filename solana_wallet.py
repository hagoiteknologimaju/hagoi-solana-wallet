...
class SolanaWalletGUI:
    ...
    COLORS = {
        "primary": "#00F5A0",  # Warna hijau neon dari index.html
        "background": "#0A0A0A",  # Dark background dari index.html
        "surface": "#1E1E1E",  # Card/surface color
        "text": "#FFFFFF",
        "text_secondary": "#A0A0A0",
        "success": "#00F5A0",  # Menggunakan warna hijau neon untuk success
        "error": "#FF3D00",
        "hover": "#00C080",  # Warna hover yang lebih gelap dari primary untuk efek yang lebih nyaman
    }
    ...
    def update_button_icons(self):
        ...
        try:
            ...
            # Update view private key button dengan ikon
            self.view_key_btn.configure(
                image=self.icons.get("key"), text="", font=ctk.CTkFont(size=13)
            )
        except Exception as e:
            print(f"Error updating button icons: {str(e)}")

    def __init__(self):
        self.wallet = SolanaWallet(network="mainnet")
        ...
        self.window.iconbitmap(resource_path(os.path.join("icons", "logo.ico")))
    ...
    def _load_and_apply_icons(self):
        ...
        try:
            icon_types = ["wallet", "send", "receive", "chart", "copy", "solana", "plus", "key"]
            icon_size = (24, 24)  # Define standard icon size
            loaded_icons = {}

            for icon_type in icon_types:
                try:
                    icon_path = resource_path(os.path.join("icons", f"{icon_type}.png"))
    ...
    def setup_ui(self):
        ...
        # Frame untuk public key label dengan grid layout
        key_label_frame = ctk.CTkFrame(key_frame, fg_color="transparent")
        key_label_frame.pack(fill="x", expand=True)
        key_label_frame.grid_columnconfigure(0, weight=1)  # Kolom untuk public key
        key_label_frame.grid_columnconfigure(1, weight=0)  # Kolom untuk buttons

        # Public key label
        self.public_key_label = ctk.CTkLabel(
            key_label_frame,
            text="Public Key: None",
            wraplength=200,  # Kurangi wraplength agar tidak terlalu panjang
            font=ctk.CTkFont(size=12),
            text_color=self.COLORS["text"],
            justify="left",
            anchor="w"
        )
        self.public_key_label.grid(row=0, column=0, sticky="w", padx=(0, 10))

        # Frame untuk tombol-tombol
        button_frame = ctk.CTkFrame(key_label_frame, fg_color="transparent")
        ...
        self.copy_key_btn.pack(side="left", padx=(0, 8))

        # View Private Key button
        self.view_key_btn = ctk.CTkButton(
            button_frame,
            text="",
        ...
        self.create_wallet_btn = ctk.CTkButton(
            ...
            hover_color=self.COLORS["hover"],  # Gunakan warna hover yang konsisten
            font=ctk.CTkFont(size=13, weight="bold"),
    ...
    def create_new_wallet(self):
        ...
        normal_btn = ctk.CTkButton(
            main_frame,
            text="Create with Hagoi (Recommended)",
    ...
    def _import_from_file(self):
        ...
        if not file_path:
            ...
            self.window.after(10000, self._clear_status_after_delay)
            return
    ...
    def _clear_status_after_delay(self, delay=10000):
    ...
    def load_and_place_background_image(self, event=None, force_reload=False):
        ...
        try:
            ...
            if rand == 0:
                background_image_path = resource_path(os.path.join("background", "bg1.png"))
            else:
                background_image_path = resource_path(os.path.join("background", "bg2.jpg"))
    ...
    def show_private_key(self):
        """Show private key in a secure dialog"""
        if not self.current_wallet:
            self.status_label.configure(text="No wallet loaded", text_color="orange")
            self.window.after(10000, self._clear_status_after_delay)
            return

        # Cek apakah wallet berasal dari file .hagoi (memiliki salt dan encrypted_key)
        is_encrypted_wallet = "salt" in self.current_wallet and "encrypted_key" in self.current_wallet

        # Hanya minta password jika wallet dienkripsi
        if is_encrypted_wallet:
            result = self.show_password_dialog(action="import")
            if result["cancelled"] or not result["password"]:
                self.status_label.configure(text="Operation cancelled", text_color="orange")
                self.window.after(10000, self._clear_status_after_delay)
                return

        # Create dialog to show private key
        dlg = ctk.CTkToplevel(self.window)
        dlg.title("Private Key")
        dlg.minsize(360, 400)
        dlg.transient(self.window)
        dlg.grab_set()
        ...
        warning_label = ctk.CTkLabel(
            main_frame,
            text="WARNING: Never share your private key with anyone!",
            font=ctk.CTkFont(size=14, weight="bold"),
            text_color=self.COLORS["error"],
            wraplength=320,
        )
        ...
        key_label = ctk.CTkLabel(
            key_frame,
            text=self.current_wallet["private_key"],
            font=ctk.CTkFont(size=14),
            text_color=self.COLORS["text"],
            wraplength=300,
        )
        key_label.pack(pady=15, padx=15)

        # Copy button
        def copy_private_key():
            self.window.clipboard_clear()
            self.window.clipboard_append(self.current_wallet["private_key"])
            copy_btn.configure(text="Copied!")
            dlg.after(2000, lambda: copy_btn.configure(text="Copy to Clipboard"))
